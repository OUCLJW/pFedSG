...
from flcore.servers.serverCD import ServerCD
from flcore.servers.serverPA import ServerPA
from flcore.servers.serveravgpre import FedAvgpre
    ...
    for i in range(args.prev, args.times):
        ...
        #在不同的模型下不同的数据集处理方法不同，调用的函数也不同
            ...
            if "mnist" in args.dataset:
                ...
                args.model = DNN(num_classes=args.num_classes).to(args.device)
            else:
                args.model = DNN(60, 20, num_classes=args.num_classes).to(args.device)
        elif  model_str == "CifarNet":
            args.model = CifarNet(num_classes=args.num_classes).to(args.device)
        ...
        if args.algorithm == "FedAvg":
        ...
        elif args.algorithm == "testavg":
            args.head = copy.deepcopy(args.model.fc)
            args.model.fc = nn.Identity()
            args.model = BaseHeadSplit(args.model, args.head)
            server = FedAvgpre(args, i)
            ...
            server = ServerCD(args, i)

        elif args.algorithm == "PA":
            server = ServerPA(args, i)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-algo', "--algorithm", type=str, default="Ditto")
    ...
    #CD
    parser.add_argument('-NCOM', "--n_components", type=int, default=150)
    parser.add_argument('-k-krun', "--kr", type=int, default=5)
